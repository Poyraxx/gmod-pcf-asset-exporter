...
def copy_file(src: Path, dst: Path, errors: list):
    try:
        if not src.exists():
            errors.append(f"Bulunamadı: {src}")
            return False
        ...
        errors.append(f"Kopyalanamadı: {src} -> {dst} | {e}")
        return False
...
def copy_asset_to_package(asset: str, roots: dict, package_dir: Path, errors: list):
    ...
    if src is None:
        errors.append(f"Kök klasör yok: {asset}")
        return False
    ...
    if asset.startswith("materials/"):
        ...
        errors.append(f"Bilinmeyen asset tipi: {asset}")
        return False
...
def ask_path(title: str, required: bool = False):
    while True:
        ...
        if not path.is_dir():
            print(f"[HATA] Bu bir klasör değil: {path}")
            continue
...
def main():
    ...
    print("========================================")
    print()
    print("Her PCF için ayrı klasör oluşturur.")
    print("PCF'in kullandığı material, model ve sound dosyalarını içine kopyalar.")
    print("Orijinal dosyalara dokunmaz.")
    print()

    pcf_root = ask_path("PCF klasör yolunu gir: ", required=True)

    print()
    materials_root = ask_path("Materials klasör yolunu gir, yoksa boş bırak: ", required=False)
    models_root = ask_path("Models klasör yolunu gir, yoksa boş bırak: ", required=False)
    sound_root = ask_path("Sound klasör yolunu gir, yoksa boş bırak: ", required=False)

    default_output = pcf_root.parent / "__pcf_asset_exports"

    print()
    output_value = input(f"Çıktı klasörü [{default_output}]: ").strip().strip('"')
    ...
    print("\n========== INDEX ==========")
    print(f"Material dosyası: {len(all_assets['materials'])}")
    print(f"Model dosyası: {len(all_assets['models'])}")
    print(f"Sound dosyası: {len(all_assets['sound'])}")

    pcfs = sorted(pcf_root.rglob("*.pcf"))

    print("\n========== PCF ==========")
    print(f"Bulunan PCF sayısı: {len(pcfs)}")

    if not pcfs:
        print("\n[HATA] Bu klasörde PCF bulunamadı.")
        pause()
        return
